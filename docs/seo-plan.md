...
- [x] **2.1 FAQ section on the homepage** (branch `faq`) — 12 questions (copy round 2, 2026-09-24: removed what-is/waterproof/cost/social-sharing; added personal AI coach, compare with other surfers & pros, coaches & crews (future tense), beginner reworded, plain-language measurement answer), between RIPPLsocial and the tester CTA. Final layout: centered — oversized "FAQ." display heading (sun full stop, hidden "Frequently asked questions" for screen readers/search) over a single 860px column. Earlier rounds (two-column, numbered, grouped, cards) kept in `archive/faq-options.html` and `archive/faq-layouts.html`
